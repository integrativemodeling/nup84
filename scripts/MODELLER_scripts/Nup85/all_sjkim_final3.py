...
aln = alignment(env)
aln.append(file='all_align_begin3.ali')
aln.write(file='all_align_final3.ali', alignment_format='PIR')
aln.write(file='all_align_final3.pap', alignment_format='PAP')
aln.id_table(matrix_file='all_align_final3.mat')
aln.check()
...
#       Use a harmonic potential and X-Y distance group.
        ...
        rsr.add(forms.upper_bound(group=physical.xy_distance,
                               feature=features.distance(at['CA:5'],
                                                         at['CA:33']),
                               mean=17.0, stdev=5.0))

        rsr.add(forms.upper_bound(group=physical.xy_distance,
                               feature=features.distance(at['CA:149'],
                                                         at['CA:203']),
                               mean=17.0, stdev=5.0))

        rsr.add(forms.upper_bound(group=physical.xy_distance,
                               feature=features.distance(at['CA:187'],
                                                         at['CA:202']),
                               mean=17.0, stdev=5.0))
        ...
        #rsr.add(forms.upper_bound(group=physical.xy_distance,
        #                       feature=features.distance(at['CA:39'],
        #                                                 at['CA:60']),
        #                       mean=16.0, stdev=2.0))
...
for files in os.listdir('.'):
    if fnmatch.fnmatch(files, 'ScNup85C2.B*.pdb'):
        print(files)
        mdl = model(env, file=files)
        mdl.rename_segments('A', 532)
        mdl.write(files)
